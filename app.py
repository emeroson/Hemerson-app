...
#
#   Configurez ces 5 valeurs dans : App Settings → Secrets (format TOML)
#
...
#
#   En local : créez .streamlit/secrets.toml avec le même contenu.
...
_ADMIN_PLAIN   = st.secrets["ADMIN_PASSWORD"]
...
def _get_auth_url() -> str:
    """
    Génère l'URL Google OAuth avec state CSRF.
    PKCE retiré : Google refuse code_challenge pour les clients de type 'web'
    → cause directe du 403 silencieux (Google bloque avant l'écran de choix de compte).
    L'endpoint v2 est utilisé : https://accounts.google.com/o/oauth2/v2/auth
    """
    # On encode quand même un token aléatoire dans le state pour le CSRF
    ...
    print(f"[OAuth DEBUG] URL générée. endpoint v2 | state CSRF OK | PKCE retiré")
    return auth_url
...
def _exchange_code(code: str, state: str) -> dict | None:
    ...
    try:
        ...
        print(f"[OAuth DEBUG] Etape 3 — fetch_token (sans PKCE, client web)")
        flow.fetch_token(
            code = code,
        )
